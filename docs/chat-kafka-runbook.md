...
정상 경로는 Kafka와 백엔드를 기동한 뒤 아래 smoke runner로 한 번에 확인할 수 있다. 활성 채팅방의
두 참여자 토큰을 로컬 환경에서 짧게 발급해 두 소켓 수신, `chat.message` offset 증가, REST 저장을
검증하며 테스트 메시지와 방의 마지막 메시지 상태는 종료 전에 복구한다.

```bash
pnpm verify:chat-kafka
```
