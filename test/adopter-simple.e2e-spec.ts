...
describe('Adopter API E2E Tests (Simple)', () => {
  ...
  describe('후기 작성', () => {
    ...
    it('POST /api/adopter/review - 후기 작성 성공', async () => {
      const reviewData = {
        ...
        content: '정말 좋은 브리더입니다. 전문적이고 친절하게 도와주셨어요. 반려동물도 건강하고 행복해 보였습니다.',
      };
  ...
  describe('즐겨찾기', () => {
    it('POST /api/adopter/favorite - 즐겨찾기 추가 성공', async () => {
      ...
      expect(response.body.success).toBe(true);
      expect(response.body.message).toContain('즐겨찾기에 추가되었습니다');
    ...
    it('DELETE /api/adopter/favorite/:breederId - 즐겨찾기 삭제 성공', async () => {
      ...
      expect(response.body.success).toBe(true);
      expect(response.body.message).toContain('즐겨찾기에서 삭제되었습니다');
  ...
  describe('신고', () => {
    it('POST /api/adopter/report - 브리더 신고 성공', async () => {
      ...
      expect(response.body.success).toBe(true);
      expect(response.body.message).toContain('신고가 접수되었습니다');
