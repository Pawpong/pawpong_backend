...
describe('BreederManagementMyReviewMapperService', () => {
    ...
    it('adopter name > nickname > 익명 순으로 결정한다', () => {
        const r1 = service.toItem({ _id: 'r-1', adopterId: { _id: 'a-1', name: '이름' }, content: 'c', writtenAt: new Date(), isVisible: true } as any);
        expect(r1.adopterName).toBe('이름');
        const r2 = service.toItem({ _id: 'r-1', adopterId: { _id: 'a-1', nickname: '닉' }, content: 'c', writtenAt: new Date(), isVisible: true } as any);
    ...
    it('isReported이면 reportCount=1, 아니면 0', () => {
        const r1 = service.toItem({ _id: 'r-1', content: 'c', writtenAt: new Date(), isVisible: true, isReported: true } as any);
        expect(r1.reportCount).toBe(1);
        const r2 = service.toItem({ _id: 'r-1', content: 'c', writtenAt: new Date(), isVisible: true, isReported: false } as any);
