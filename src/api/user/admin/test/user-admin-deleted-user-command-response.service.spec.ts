...
describe('사용자 관리자 탈퇴 사용자 명령 응답 서비스', () => {
    const service = new UserAdminDeletedUserCommandResponseService();

    it('복구 응답을 만든다', () => {
        expect(
            service.toRestoreDeletedUserResponse('user-1', 'adopter', 'deleted', new Date('2026-04-09T01:00:00.000Z')),
        ).toEqual({
            userId: 'user-1',
            role: 'adopter',
    ...
    it('영구 삭제 응답을 만든다', () => {
        ...
        expect(service.toHardDeleteUserResponse('user-2', 'breeder', '브리더', 'breeder@test.com')).toMatchObject({
            userId: 'user-2',
            role: 'breeder',
            userName: '브리더',
