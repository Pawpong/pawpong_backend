...
import { UploadAdminCommandResponseMessageService } from './domain/services/upload-admin-command-response-message.service';
...
export class UploadAdminFolderDeleteController {
    constructor(
        ...
        private readonly uploadAdminCommandResponseMessageService: UploadAdminCommandResponseMessageService,
    ) {}
    ...
    async deleteFolder(@Query('folder') folder: string): Promise<ApiResponseDto<DeleteFilesResponseDto>> {
        ...
        return ApiResponseDto.success(result, this.uploadAdminCommandResponseMessageService.folderDeleted(folder));
    }
}
