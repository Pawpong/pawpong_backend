...
describe('입양자 계정 탈퇴 유스케이스', () => {
    ...
    it('입양자 정보가 없으면 BadRequestException을 던진다', async () => {
        ...
        await expect(useCase.execute('user-1', { reason: 'already_adopted' })).rejects.toThrow(BadRequestException);
        await expect(useCase.execute('user-1', { reason: 'already_adopted' })).rejects.toThrow('입양자 정보를 찾을 수 없습니다.');
    });

    it('이미 탈퇴한 계정이면 BadRequestException을 던진다', async () => {
        adopterAccountCommandPort.findAdopterById.mockResolvedValue({
    ...
    it("reason이 'other'이고 otherReason이 없으면 BadRequestException을 던진다", async () => {
        adopterAccountCommandPort.findAdopterById.mockResolvedValue({
            accountStatus: 'active',
        });

        await expect(useCase.execute('user-1', { reason: 'other', otherReason: undefined })).rejects.toThrow(BadRequestException);
    ...
    it('탈퇴 처리 중 오류 발생 시 BadRequestException을 던진다', async () => {
        adopterAccountCommandPort.findAdopterById.mockResolvedValue({
            accountStatus: 'active',
        });
        ...
        await expect(useCase.execute('user-1', { reason: 'already_adopted' })).rejects.toThrow(BadRequestException);
        await expect(useCase.execute('user-1', { reason: 'already_adopted' })).rejects.toThrow('회원 탈퇴 처리 중 오류가 발생했습니다.');
    });
});
