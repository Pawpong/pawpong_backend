...
import { BreedAdminResultMapperService } from '../../../../service/breed/domain/services/breed-admin-result-mapper.service';
...
export class UpdateBreedUseCase {
    ...
    async execute(id: string, dto: UpdateBreedCommand): Promise<BreedAdminItemResult> {
        ...
        if (dto.category && dto.category !== breed.category) {
            ...
            if (existing) {
                throw new DomainConflictError(`이미 ${breed.petType}의 ${dto.category} 카테고리가 존재합니다.`);
            }
        }
