...
export interface BreederManagementStandardQuestion {
    id: string;
    type: string;
    label: string;
    required: boolean;
...
@Injectable()
export class BreederManagementStandardQuestionCatalogService {
    getAll(): BreederManagementStandardQuestion[] {
        return [
            {
                id: 'privacyConsent',
                type: 'checkbox',
    ...
    getIds(): string[] {
        return this.getAll().map((question) => question.id);
    }
}
