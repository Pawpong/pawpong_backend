...
@Injectable()
export class DeleteCommunityPostCommentUseCase {
    constructor(
    ...
    async execute(commentId: string, userId: string): Promise<{ deleted: boolean }> {
        ...
        if (comment.authorId !== userId) throw new ForbiddenException('본인 댓글만 삭제할 수 있습니다.');

        const { changed } = await this.commentWriter.softDeleteCommentByAuthor(commentId, userId);
