...
import { ApiRegisterAdopterEndpoint, ApiRegisterBreederEndpoint } from '../swagger/index';
...
export class AuthSignupController {
    ...
    async registerAdopter(
        @Body() dto: RegisterAdopterFullRequestDto,
    ): Promise<ApiResponseDto<RegisterAdopterResponseDto>> {
        const result = await this.registerAdopterUseCase.execute(dto);
        return ApiResponseDto.success(result, AUTH_RESPONSE_MESSAGE_EXAMPLES.adopterSignupCompleted);
    }
    ...
    async registerBreeder(@Body() dto: RegisterBreederRequestDto): Promise<ApiResponseDto<RegisterBreederResponseDto>> {
        ...
        return ApiResponseDto.success(result, AUTH_RESPONSE_MESSAGE_EXAMPLES.breederSignupCompleted);
    }
}
