...
describe('AdopterApplicationDetailAssemblerService', () => {
    ...
    it('breeder가 있으면 이름을 사용한다', () => {
        const result = service.toResponse(application, { name: '브리더A' } as any, 'review-1');
    ...
    it('breeder가 null이면 알 수 없음을 사용한다', () => {
        const result = service.toResponse(application, null, null);
        expect(result.breederName).toBe('알 수 없음');
    });

    it('processedAt이 없으면 undefined', () => {
        const result = service.toResponse({ ...application, processedAt: undefined }, null, null);
        expect(result.processedAt).toBeUndefined();
    });

    it('customResponses가 없으면 빈 배열', () => {
        const result = service.toResponse({ ...application, customResponses: undefined }, null, null);
