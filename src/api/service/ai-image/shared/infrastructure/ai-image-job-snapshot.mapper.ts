...
export function toAiImageJobSnapshot(job: AiImageJobDocument): AiImageJobSnapshot {
    return {
        ...
        referenceImageObjectKeysSnapshot: job.referenceImageObjectKeysSnapshot ?? [],
        inputFidelitySnapshot: job.inputFidelitySnapshot ?? 'low',
        postProcessTypeSnapshot: job.postProcessTypeSnapshot ?? 'pixelate',
        pixelSizeSnapshot: job.pixelSizeSnapshot ?? 96,
        paletteSizeSnapshot: job.paletteSizeSnapshot ?? 48,
