...
export class ProfileFollowRepository {
    ...
    async findFollowEdges(
        ...
        return this.followModel
            .find(await this.toDirectionFilter(userId, direction))
            .sort({ createdAt: -1 })
            .skip(skip)
    ...
    async countFollowEdges(userId: string, direction: FollowDirection): Promise<number> {
        return this.followModel.countDocuments(await this.toDirectionFilter(userId, direction)).exec();
    }
    ...
    private async toDirectionFilter(userId: string, direction: FollowDirection) {
        if (!isIosAppRequest()) return direction === 'followers' ? { followeeId: userId } : { followerId: userId };
        const consentedIds = (await eligibleAppAuthorIds(this.followModel.db)).map(String);
        return direction === 'followers'
            ? { followeeId: userId, followerId: { $in: consentedIds } }
            : { followerId: userId, followeeId: { $in: consentedIds } };
    }
