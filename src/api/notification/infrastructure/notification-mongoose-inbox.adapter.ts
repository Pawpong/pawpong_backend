...
export class NotificationMongooseInboxAdapter implements NotificationInboxPort {
    ...
    async findPagedByUser(
        ...
        return this.notificationRepository.findPagedByUser(userId, options) as Promise<{
    ...
    findByIdForUser(userId: string, notificationId: string): Promise<NotificationInboxRecord | null> {
        return this.notificationRepository.findByIdForUser(userId, notificationId) as Promise<NotificationInboxRecord | null>;
    }

    markAsRead(userId: string, notificationId: string, readAt: Date): Promise<NotificationInboxRecord | null> {
        return this.notificationRepository.markAsRead(userId, notificationId, readAt) as Promise<NotificationInboxRecord | null>;
    }
